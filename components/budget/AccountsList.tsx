    ...
    <>
      <div className="bg-white rounded-lg border p-4 h-full flex flex-col">
        <div className="flex items-center justify-between mb-4">
          <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">Bank Accounts</div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          {accounts.length === 0 ? (
            <div className="text-sm text-slate-500 dark:text-slate-300">No accounts yet. Add your first account.</div>
          ) : (
            accounts.map((acc) => (
              <div
                key={acc.id}
                className="p-3 border dark:border-slate-800 rounded-md bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="font-medium text-sm text-slate-900 dark:text-slate-50">{acc.name}</div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-900 rounded-lg p-6 w-full max-w-sm">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">Add Account</h2>
              <button
                type="button"
                onClick={() => {
